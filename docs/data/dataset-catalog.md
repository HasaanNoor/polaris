...
## Curated UNESCO UIS Education Panel

Phase 17 adds a derived reviewed dataset, `UNESCOEducationPanel`, built from local UNESCO UIS files under `data/raw/unesco/`. DEM, SDG, SCN-SDG, and SDG11 are profiled, but only reviewed SDG national education indicators are promoted into the default panel. DEM is demographic/economic support data, SCN-SDG is science/R&D, and SDG11 is cultural heritage expenditure, so they are not integrated as education variables.

Current local status: UNESCO files are downloaded locally; SDG national data is profiled and integrated for 15 reviewed education indicators; 2,205 education-like SDG indicators are deferred with machine-readable reasons. The panel exports `examples/unesco/unesco_education_panel_sample.csv`, `examples/unesco/unesco_education_panel_manifest.json`, `examples/unesco/unesco_education_quality_summary.json`, `examples/unesco/unesco_integrated_variables.json`, and `examples/unesco/unesco_deferred_indicators.json`.
...
| UNESCO Institute for Statistics | UIS education data | education, human development | Broad global country coverage, subject to national reporting gaps | Country-year education indicator series | Provider file or API-derived tabular snapshot | Round 1 | Implemented | Downloaded locally | Integrated | Phase 17 integrates reviewed SDG education indicators; subgroup-heavy and learning-outcome variants remain deferred. |
...
Current Phase 10 built-in adapters exist only for World Bank WDI, WHO GHO, and UNESCO UIS. This catalog distinguishes adapter availability from actual local status; WDI, WHO panel subsets, WGI, and the reviewed UNESCO education panel have local integrations, while other selected datasets remain planned.
