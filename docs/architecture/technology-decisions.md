...
Phase 8 uses the Phase 1 Python schema foundation, the Phase 2 deterministic in-memory dataset registry, the Phase 3 local CSV ingestion layer, the Phase 4 deterministic statistical analysis package, the Phase 5 deterministic structured evidence package, the Phase 6 deterministic domain-agent package, the Phase 7 deterministic coordination package, and a guardrailed synthesis package. Python 3.12 or a compatible modern Python version is the supported runtime target, with Pydantic v2 used for typed validation and JSON serialization. The ingestion loader uses Python's standard `csv` module and `hashlib` for SHA-256 checksums. NumPy and SciPy are adopted for Phase 4 numerical arrays, correlations, distribution statistics, and linear-algebra-backed OLS diagnostics. Phases 5, 6, 7, and deterministic Phase 8 behavior use standard-library SHA-256 hashing for deterministic evidence, claim, assessment, coordinated-assessment, agreement, divergence, gap, and deterministic synthesis IDs. Phase 8 adds no new runtime dependencies and exposes only a small mockable provider protocol for optional LLM synthesis. Pytest is used for schema, registry, ingestion, analysis, evidence, agent, coordination, and synthesis tests. Ruff is configured for linting and formatting checks only.
